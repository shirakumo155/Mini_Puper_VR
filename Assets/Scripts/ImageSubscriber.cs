...
{
    // 受信するROSのトピック名
    [SerializeField] string rosTopicName = "camera/rgb/image_raw/compressed";
    ...
    void Start()
    {
        ...
        ros.Subscribe<CompressedImageMsg>(rosTopicName, ImageMsgUpdate);
    ...
    void ImageMsgUpdate(CompressedImageMsg rawImage)
    {
        ...
        if (isDebugMode)
        {
            ...
            Debug.Log("rawImage data length :" + rawImage.data.Length);
        }
    }
